...
if (isset($_POST['login']) && isset($_POST['password']) && isset($_POST['email']) && isset($_POST['age'])) {
   if (($_POST['login']  != '') && ($_POST['age'] >6) &&($_POST['age'] <151) && (preg_match("(\w+@[a-z_]+?\.[a-z]{2,})", strtolower($_POST['email'])) == 1) && (preg_match("((?=.*\d)(?=.*[a-z])(?=.*[A-Z]).{8,})", ($_POST['password'])) ==1)) {
      ...
      if ($c == 0) {
          ...
          if ($data->execute([$login, $pass, $email, $age, "user"])) {
              ...
              if ($data->execute([$login,$email])) {
                  ...
                  echo $c;
                  $_SESSION['user_id'] = $c;
                  header("Refresh: 5; url=http://f0506120.xsph.ru/index.php");
              } else {
                  $error = true;
          ...
          $error = true;
          $msg = 'Пользователь с таким логином/почтой уже существует';
      }
   } else {
       $error = true;
       $msg = 'Введены некоретные даные или возраст не соотвествует требованиям';
   }
} else {
    $error = true;
    $msg = 'Отсутствуют логин/пароль/почта/не указан возрат';
}
...
<html lang="ru">
    ...
    <div class="container">
        ...
        if ($error) {
            ...
            echo "Регестрция успешна. Через 5 секунд Вы перенесётись на главную страницу";
            echo '</div>';
        }
        ?>
            ...
            <div class="col-4">
                <form action="registration.php" method="post">
                    ...
                    <div class="form-group">
                        ...
                        <input type="number" class="form-control" id="InputAge" min="1" max="150" name="age" required>
                    </div>
